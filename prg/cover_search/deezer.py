        ...
        if data["total"] > 0:
            ...
            try:
                album_cover = album_info[
                    "cover_big"
                ]  # URL de la couverture en grande taille
                # logging.info("Deezer album url : %s",album_cover)
                return album_cover
            except Exception as e:
                print(
                    f"Erreur lors de la récupération du nom de l'album avec Deezer. {e}"
                )
...
# album_cover_url = get_deezer_title("Roy Hargrove", "Diamond in the Rough")
album_cover_url = get_deezer_album("Lalo Schifrin", "Mission: Impossible... And More!")
